...
- Packaged as `leaguepipeline` via `pyproject.toml` (src-layout, `src/leaguepipeline/`) —
  install with `pip install -e ".[test]"`. `requirements.txt`/`pytest.ini` no longer exist,
  replaced by `pyproject.toml`'s `dependencies`/`optional-dependencies.test`/`tool.pytest.ini_options`.
...
- Reference-data seeding: `python -m leaguepipeline.seed_reference_data` (Data Dragon champions/items) — see [[schema-pipeline-debt]] for a bug that currently breaks this.
...
- CI: GitHub Actions (`.github/workflows/ci.yml`) runs `pytest` on push/PR with dummy `RIOT_API_KEY`/`DATABASE_URL` env values (tests mock all HTTP/DB calls).
